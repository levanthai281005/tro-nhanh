...
- **Địa chỉ:** `provinceCode` và `wardCode` phải có trong **danh mục gói sẵn** (`packages/constants`), và `wardCode` phải thuộc đúng `provinceCode`; **tên phường/xã do máy chủ suy ra từ mã, không nhận từ client** — client chỉ gửi mã + địa chỉ chi tiết (BR-043).
...
- **UtilityReading:** chỉ số lưu **số nguyên** (BR-041); `currentReading ≥ previousReading` **khi nước tính theo khối**, **bỏ trống khi tính theo đầu người hoặc khoán** (BR-042); `unitPrice ≥ 0` và **chép từ đơn giá đang áp dụng lúc ghi** (giá phòng, trống thì giá khu — không đọc ngược, BR-036); `pricingMethod` chép từ cách tính của khu lúc ghi, `occupantCount` > 0 và bắt buộc khi tính theo đầu người (lấy từ tổng nhân khẩu người ở đang ở phòng, không nhập tay); unique (roomId, type, period).
