# Getting Started

Dựng dự án trên máy mới: đọc [development setup](./docs/DEVELOPMENT_SETUP.md) — danh sách
đúng những thứ cần cài, và những thứ **không** cần (không có `.env`, không cần backend).

Đọc [README](./README.md) để cài đặt và chạy dự án, sau đó đọc
[source code guidelines](./SOURCE_CODE_GUIDELINES.md) để hiểu cấu trúc workspace. Trước khi viết
mã, đọc thêm [workspace guidelines](./.agents/AGENTS.md),
[coding standards](./.agents/rules/CODING_STANDARDS.md) và
[naming conventions](./.agents/rules/NAMING_CONVENTIONS.md).

## Reference Documentation

- [Next.js 16 documentation](https://nextjs.org/docs/16)
- [Next.js App Router](https://nextjs.org/docs/16/app)
- [React documentation](https://react.dev/)
- [Expo SDK 57 documentation](https://docs.expo.dev/versions/v57.0.0/)
- [Expo Router](https://docs.expo.dev/versions/v57.0.0/router/introduction/)
- [Expo Font](https://docs.expo.dev/versions/v57.0.0/sdk/font/)
- [Expo Image](https://docs.expo.dev/versions/v57.0.0/sdk/image/)
- [Expo SecureStore](https://docs.expo.dev/versions/v57.0.0/sdk/securestore/)
- [Expo SplashScreen](https://docs.expo.dev/versions/v57.0.0/sdk/splash-screen/)
- [TypeScript documentation](https://www.typescriptlang.org/docs/)
- [pnpm workspaces](https://pnpm.io/workspaces)
- [Turborepo documentation](https://turborepo.com/docs)
- [Tailwind CSS v3 documentation](https://v3.tailwindcss.com/docs)
- [NativeWind v4 documentation](https://www.nativewind.dev/v4/overview/)
- [TanStack Query documentation](https://tanstack.com/query/latest/docs/framework/react/overview)
- [Zod documentation](https://zod.dev/)
- [React Hook Form documentation](https://react-hook-form.com/get-started)
- [Axios documentation](https://axios-http.com/docs/intro)

## Guides

- [Server and Client Components](https://nextjs.org/docs/16/app/getting-started/server-and-client-components)
- [Fetching Data](https://nextjs.org/docs/16/app/getting-started/fetching-data)
- [Mutating Data](https://nextjs.org/docs/16/app/getting-started/mutating-data)
- [Caching](https://nextjs.org/docs/16/app/getting-started/caching)
- [Revalidating Data](https://nextjs.org/docs/16/app/getting-started/revalidating)
- [Image Optimization](https://nextjs.org/docs/16/app/getting-started/images)
- [Font Optimization](https://nextjs.org/docs/16/app/getting-started/fonts)
- [Expo monorepos](https://docs.expo.dev/guides/monorepos/)
- [Expo environment variables](https://docs.expo.dev/guides/environment-variables/)
- [Expo authentication](https://docs.expo.dev/guides/authentication/)
- [OpenAPI TypeScript](https://openapi-ts.dev/introduction)

Tài liệu Next.js đúng phiên bản cài đặt cũng có sẵn tại
`apps/web/node_modules/next/dist/docs/`; ưu tiên tài liệu này nếu hành vi giữa các phiên bản
khác nhau.
